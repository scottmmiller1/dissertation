...
rtitle("Members (count)"\"Revenue (USD)"\"Costs (USD)"\"Net revenue (USD)"\"Revenue per member (USD)"\"Net revenue per member (USD)"\"Goat revenue (USD)"\"Planning time horizon (years)"\"Number of ICT assets (count)"\"Number of non-ICT assets (count)")replace
...
ctitle("","N","Mean","sd","Min","Max") ///
rtitle("Age (years)"\"Literacy (0/1)"\"Number of SHG meetings attended (count)"\"Received Sale Information (0/1)"\"Goats sold through cooperative (count)"\"Total goats sold (count)"\"Revenue per goat (USD)"\"Revenue per goat through cooperative (USD)"\"Net goat income (USD)"\"Transparency discrepancy index") replace
